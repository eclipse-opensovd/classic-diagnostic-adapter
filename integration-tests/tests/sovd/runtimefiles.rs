...
/// authoritative across a restart, so the deleted databases are not picked up
/// again from the configured database directory.
#[tokio::test]
async fn applied_empty_snapshot_survives_restart() -> Result<(), TestingError> {
    ...
    // The restart finds an empty collection in storage and must not fall back to
    // the configured database directory, so it needs a configuration that
    // tolerates an empty vehicle.
        ...
        || async {
            assert!(
                get_file_list(&runtime.config, &auth, RUNTIMEFILES_CURRENT)
                    ...
                    .items
                    .is_empty(),
                "the restart must not repopulate the emptied collection"
            );
            send_cda_request(
                &runtime.config,
                ECU_FLXC1000_ENDPOINT,
