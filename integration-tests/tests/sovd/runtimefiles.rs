...
/// releases the lock on every path (`Ok`, `Err`, panic) and waits out an active
/// runtime update protection at both ends. Today the release is skipped
/// whenever a test returns early, and the leaked lock then fails every later
/// test that asserts on lock ownership.
...
///
/// Apply is a *snapshot swap*: the staged collection replaces the active one
/// and anything missing from it is dropped. The usual shortcut - upload a
/// single MDD and let the plugin seed the rest of the staging collection from
/// the active one (`init_collection_from_copy_if_missing`) - only works when
/// `runtimefiles-current` is populated, and on a fresh test container it is
/// not: the CDA falls back to loading MDDs straight from its configured
/// database directory when storage holds none (`resolve_mdd_paths`). There is
/// nothing to copy, so a one-file upload applies a one-ECU snapshot, and every
/// later test in the shared suite then talks to a vehicle that lost five ECUs.
///
...
///
/// Read from disk rather than hard-coded: a fixture added later has to end up in
/// the staged snapshot too, and the failure mode of missing one is an ECU that
/// silently disappears from the vehicle.
...
/// Helper: POSTs an execution mode to the executions endpoint (expects 202
/// Accepted) and waits for that execution to finish.
...
/// has been lifted.
///
/// An execution is asynchronous: the POST only answers 202. Waiting for the
/// execution resource to report `completed` is still not enough, because the
/// protection outlives that status - the update task publishes the status,
/// then re-enables communication (transport, lifecycle hooks and whole-vehicle
/// variant detection, on the order of a second), and only then drops the
/// protection. Until it does, every non-exempt route answers `409 Update in
/// progress`, including `DELETE /vehicle/v15/locks/{id}`: a test that returns
/// inside that window cannot even release its own vehicle lock, and hands the
/// 409 to whichever test runs next.
///
/// The execution resource itself stays readable throughout, because it is on
/// the protection's exempt list.
async fn wait_for_execution_completion(
    ...
    // `runtimefiles-current` is not exempt, so it answers 409 for exactly as
    // long as the protection is installed.
...
async fn runtimefiles_apply_blocked_by_vehicle_and_ecu_lock() -> Result<(), TestingError> {
    ...
    // Stage the whole database rather than a single MDD: Apply is a snapshot
    // swap, so a one-file upload would leave the shared CDA serving a one-ECU
    // vehicle for every test that runs after this one.
    ...
    // No Rollback afterwards: the staged snapshot *was* the active database, so
    // there is nothing to restore, and on a pristine container the backup Apply
    // takes is empty - Rollback answers 404, which used to abort this test
    // before it released the vehicle lock below.
    execute_mode(&runtime.config, &auth, ExecutionMode::Apply).await?;
