...
use crate::{
    ...
    util::{
        ...
        test_env::{TestEnv, setup_integration_test, wait_for_ecus_online},
    },
};
...
async fn storage_file_bytes(env: &TestEnv, collection: &str) -> Result<Vec<u8>, TestingError> {
    env.read_cda_storage_file(&format!("collections/{collection}/flxc1000.mdd"))
        .await
}
...
async fn failed_real_ecu_data_build_restores_live_data_and_mdd_bytes() -> Result<(), TestingError> {
    let mut runtime = setup_integration_test().await?;
    let auth = auth_header(&runtime.config, None).await?;
    ...
    let original_bytes = storage_file_bytes(&runtime, "diagnostic_database").await?;
    ...
    runtime
        .restart_cda_with_config(require_flxc1000_in_config)
        .await?;
    wait_for_ecus_online(&runtime.config).await?;
    let vehicle_lock_id = setup_with_lock(&runtime.config, &auth).await;
    ...
    assert_eq!(
        storage_file_bytes(&runtime, "diagnostic_database").await?,
        original_bytes
    );
    send_cda_request(
        &runtime.config,
...
async fn failed_real_rollback_build_restores_live_data_and_mdd_bytes() -> Result<(), TestingError> {
    ...
    stage_full_database(&runtime.config, &auth).await?;
    execute_mode(&runtime.config, &auth, ExecutionMode::Apply).await?;
    let live_bytes = storage_file_bytes(&runtime, "diagnostic_database").await?;
    teardown_lock(&runtime.config, &auth, &vehicle_lock_id).await;
