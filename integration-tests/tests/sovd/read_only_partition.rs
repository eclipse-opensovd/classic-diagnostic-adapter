...
async fn cda_should_work_on_a_read_only_partition() {
    let cda = run_cda_container().await
        .with_readonly_rootfs(true);

    let cda = {
        let databases_dir_on_host = PathBuf::from("../testcontainer/odx")
            .canonicalize().unwrap(); //need absolute path
        let databases_dir_in_container = "/odx";

        cda
            .with_cmd(["--databases-dir", databases_dir_in_container])
            .with_mount(
                Mount::bind_mount(
...
pub async fn run_cda_container() -> CdaContainer {
    ...
    let thread_name = thread::current().name().unwrap()
        .replace("::", ".");
    image = image
        .with_container_name(format!("cda-integration-test-{thread_name}"));
