...
/// The running CDA's task, awaited by [`stop_cda`] so callers only observe
/// the instance as stopped once its port is actually released - otherwise a
/// following [`start_cda`] can race the old instance for the same port.
///
/// A plain `std::sync::Mutex` is enough: every access is a synchronous
/// set/take that never holds the guard across an `.await`.
...
///
/// The CDA outlives a single test, so a test inherits whichever instance the
/// previous one left behind. This records what that instance is, so
/// [`ensure_cda_running`] can tell an unusable leftover (stopped, or running in
/// deferred mode) from the plain instance most tests expect.
...
/// Like [`setup_integration_test`], but does NOT start the CDA.
///
/// Returns the runtime infrastructure (config, `ecu_sim`) with the ECU simulator
/// running, but the CDA process/docker container is not started. The caller
/// is responsible for starting the CDA (e.g., via [`restart_cda`]).
///
/// This is useful for tests that need control over exactly when the CDA starts,
/// such as verifying no network traffic occurs before a triggering event.
...
async fn setup<'a>(
    ...
    // Every later test therefore has to establish the CDA state it needs itself,
    // rather than depending on the mode the first test happened to pick or on
    ...
    if exclusive {
        ...
        // For non-exclusive tests, just return the shared runtime reference.
        Ok((runtime, None))
    }
}
...
async fn initialize_runtime(cda: CdaStartup) -> Result<TestRuntime, TestingError> {
    ...
    match cda {
        ...
        // Without the CDA there is nothing else to wait for; in the local case
        // `start_ecu_sim_*` already waited for the sim.
...
pub(crate) async fn stop_cda() -> Result<(), TestingError> {
    ...
    sender.send(()).ok();
    // Await the task itself, not just the shutdown signal: a following
    // `start_cda` binds the same port, and would otherwise race the old
    // instance for it before the listener is actually released.
    let task = CDA_TASK
        .lock()
...
/// compose from pulling in the `cda` service that the test wants to start
/// itself.
...
/// `depends_on` targets, which would silently resurrect containers a test
/// stopped on purpose (e.g. the ecu-sim, which the `cda` service depends on).
...
/// (Re)starts the shared CDA with `config`.
///
/// Doubles as a plain start: a CDA that is not currently running is not an
/// error, so tests that deliberately left it down can come back through here.
pub(crate) async fn restart_cda(config: &Configuration) -> Result<(), TestingError> {
    mark_cda_stopped().await;
    if use_docker() {
        write_config_toml(&test_container_dir()?, config.clone())?;
        // Stop the CDA first (if it exists) to ensure a clean restart with new config.
        // `docker compose stop` is safe even if the container doesn't exist.
        let _ = docker_compose_stop("cda");
        // Then start it (creates if doesn't exist, starts if stopped).
        // `--no-deps`: the `cda` service depends_on a healthy ecu-sim, so without
        // it compose would restart a sim that a test stopped on purpose (e.g. to
        // observe the ECUs as Offline after a CDA restart).
        docker_compose_up(Some("cda".to_owned()), true)?;
    } else {
        // `stop_cda` only fails when nothing is running, which is the state we
        // are about to create anyway.
        stop_cda().await.ok();
        start_cda(config.clone());
    }
...
/// running with different [`CommunicationSettings`] (the deferred-init tests
/// swap in an on-demand configuration).
///
/// Restarting is the expensive path, so it only happens when the instance that
/// is actually up cannot serve this test.
...
/// Stops the shared CDA, leaving the ECU simulator up.
///
/// Tests that start their own CDA need the vehicle network quiet first: an
/// inherited instance would put `DoIP` traffic on the wire before they even
/// begin.
async fn stop_shared_cda() {
    ...
    if use_docker() {
        ...
        // Fails only when nothing is running, which is the desired state.
        stop_cda().await.ok();
    }
}

/// One-shot readiness probe, as opposed to [`wait_for_cda_online`]'s polling:
/// answers whether a CDA is serving *right now*.
