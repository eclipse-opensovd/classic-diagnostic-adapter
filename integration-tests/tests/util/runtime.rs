...
async fn initialize_runtime(cda: CdaStartup) -> Result<TestRuntime, TestingError> {
    ...
    let config = if use_mixed() {
        cda_test_config_mixed(host.clone(), cda_port)?
    } else if use_can() {
        cda_test_config_can(host.clone(), cda_port)?
    } else {
        cda_test_config(host.clone(), cda_port)?
    };
    ...
    start_docker_compose(cda, cda_port, sim_control_port)?;
...
fn cda_test_config(host: String, cda_port: u16) -> Result<Configuration, TestingError> {
...
fn cda_test_config_mixed(host: String, cda_port: u16) -> Result<Configuration, TestingError> {
...
fn start_docker_compose(
    ...
    write_docker_env_file(&test_container_dir, cda_port, sim_control_port)?;
...
fn docker_env_content(can: bool, cda_port: u16, sim_control_port: u16) -> String {
...
fn write_docker_env_file(
    ...
    let env_content = docker_env_content(can_infra(), cda_port, sim_control_port);
...
mod tests {
    ...
    fn docker_env_can_socketcand_lines_are_well_formed() {
        let content = docker_env_content(true, 20002, 8181);
    ...
    fn docker_env_without_can_has_no_socketcand_vars() {
        let content = docker_env_content(false, 20002, 8181);
        assert!(
            !content.contains("SOCKETCAND"),
            "unexpected CAN vars:\n{content}"
        );
