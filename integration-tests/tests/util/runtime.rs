...
fn register_cleanup() {
    extern "C" fn cleanup_handler() {
        ...
        if use_docker {
            ...
            if std::env::var(CDA_INTEGRATION_TEST_COVERAGE).is_ok_and(|s| s == "true")
...
fn extract_coverage_from_container() -> Result<(), TestingError> {
    ...
    if !stop_status.success() {
        tracing::warn!("Failed to gracefully stop CDA container for coverage extraction");
    }
    ...
    if status.success() {
        tracing::debug!("Coverage data extracted to {:?}", coverage_output_dir);
    } else {
        // Non-fatal: coverage data may not exist if tests failed early
        tracing::warn!("Failed to extract coverage data (this is expected if tests failed early)");
    }
    ...
    if status.success() {
        tracing::debug!("Instrumented binary extracted");
    }
