...
//!
//! Any type implementing [`HttpRestrictionGuard`] can be installed as a Tower
//! layer that evaluates incoming requests and either passes them through or
//! denies them with a structured SOVD error response.
//!
//! [`HttpRestrictionGuard::is_active`] is the cheap fast path. While it returns
//! `false` the middleware forwards requests without allocating the async
//! evaluation future.
//!
//! Install globally via [`install_http_restriction_guard`] on a
//! [`DynamicRouter`](crate::dynamic_router::DynamicRouter), or apply an
    ...
    S::Future: Send + 'static,
{
    ...
    fn call(&mut self, request: axum::extract::Request) -> Self::Future {
        ...
        // Tower's Service::Future must be a concrete type, which an async block
        // is not.
        Box::pin(async move {
            if !guard.is_active() {
...
/// Converts an [`HttpRestrictionDenial`] to an SOVD HTTP error response, adding
/// a `Retry-After` header when a retry hint is present.
///
...
///   with [`VendorErrorCode::CommunicationNotReady`], since SOVD defines no
///   standard code for either. The distinction rides in `denial.message`.
...
mod tests {
    ...
    async fn dropping_communication_protection_does_not_affect_update_protection() {
        ...
        // Drop communication protection, as a failed resume would.
        drop(communication);
