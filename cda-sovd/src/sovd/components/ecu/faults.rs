...
mod tests {
    ...
    use crate::sovd::test_utils::{create_test_security_plugin, create_test_webserver_state};
    ...
    async fn test_get_faults() {
        ...
        let mut expected_dtcs = HashMap::default();
        expected_dtcs.insert(test_dtc.record.code, test_dtc.clone());
        ...
        let state = create_test_webserver_state::<MockDiagServiceResponse, MockUdsEcu, MockFileManager>(
            ecu_name,
            mock_uds,
            mock_file_manager,
        );

        let query = sovd_interfaces::components::ecu::faults::get::FaultQuery {
            status: None,
            severity: None,
            scope: None,
        ...
        let security_plugin = create_test_security_plugin();
        ...
        let fault = &response_data.items[0];
        ...
        assert_eq!(status.test_failed_this_operation_cycle, Some(test_dtc.status.test_failed_this_operation_cycle));
        ...
        assert_eq!(status.test_not_completed_since_last_clear, Some(test_dtc.status.test_not_completed_since_last_clear));
        assert_eq!(status.test_failed_since_last_clear, Some(test_dtc.status.test_failed_since_last_clear));
        assert_eq!(status.test_not_completed_this_operation_cycle, Some(test_dtc.status.test_not_completed_this_operation_cycle));
        assert_eq!(status.warning_indicator_requested, Some(test_dtc.status.warning_indicator_requested));
