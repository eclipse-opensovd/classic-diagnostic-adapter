...
use crate::{
    create_schema,
    sovd::{
        ExecutionLock, FgServiceExecution, ResolvedLocks, WebserverState,
        error::{
...
impl<T: UdsEcu + Clone> FromRequestParts<WebserverState<T>> for FgContext<T> {
    ...
    async fn from_request_parts(
        ...
        // Resolved before the lock topology is read, so a request naming no live
        // group returns 404 without waiting on a topology reservation.
...
pub(crate) mod tests {
    ...
    use cda_interfaces::{ReloadComponent, UdsEcu, util::std_ext::lock_write};
    use cda_plugin_communication_management::lifecycle::enabled_communication_access_for_test;
    use uuid::Uuid;

    use super::WebserverFgState;
    use crate::sovd::{FgServiceExecution, HashMap, HashSet, SovdIdentities, SovdRegistry};
    ...
    pub async fn create_test_fg_state<T: UdsEcu + Clone>(
        uds: T,
        functional_group_name: String,
    ) -> WebserverFgState<T> {
        let lock_provider = Arc::new(crate::sovd::SovdLockStateProvider::new(Vec::new()).view());
        WebserverFgState {
            uds,
