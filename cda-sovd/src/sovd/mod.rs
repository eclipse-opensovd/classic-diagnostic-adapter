...
async fn get_components<T: UdsEcu + SchemaProvider + Clone>(
    ...
    let rootlocked_ecus = state
        .uds
        .get_ecus_with_sds(true, &components_config.rootlocked_ecus_sds)
        .await;
    let lin_ecus = state
        .uds
        .get_ecus_with_sds(true, &components_config.lin_ecus_sds)
        .await;
    let schema = if query.include_schema {
        Some(crate::sovd::create_schema!(ComponentsResponse<Resource>))
    } else {
        None
    };
    (
        ...
        Json(ComponentsResponse::<Resource> {
            ...
            rootlocked_ecus: rootlocked_ecus
                .into_iter()
                .map(ecu_to_resource)
                .collect::<Vec<_>>(),
            lin_ecus: lin_ecus
