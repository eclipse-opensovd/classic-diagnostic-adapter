...
use cda_interfaces::{
    SchemaProvider, UdsEcu, communication_control::CommunicationAccess,
    datatypes::ComponentsConfig, dlt_ctx, http_protection::registry::HttpRouteMatcher,
...
pub use crate::sovd::{
    SovdIdentities, SovdLockStateView, SovdRegistry, SovdRegistrySnapshot, SovdRegistryView,
...
{
    let vehicle_router = build_vehicle_routes::<T, S>(config, resources);
...
pub fn build_vehicle_routes<T, S>(
    config: VehicleConfig,
    resources: VehicleResources<T>,
) -> aide::axum::ApiRouter
where
...
where
    S: SecurityPluginLoader,
    P: cda_interfaces::runtime_update_api::RuntimeFileCatalog
        + cda_interfaces::runtime_update_api::RuntimeFileStore
        + cda_interfaces::runtime_update_api::RuntimeUpdateExecutor,
                ...
                routing::get(move |ExtractHost(host): ExtractHost| {
                    ...
                    async move {
                        ...
                        openapi::expand_templated_paths(
                            &mut api,
                            live.ecu_index().iter(),
                            live.functional_group_index().iter(),
                        );
...
mod tests {
    ...
    use super::{OPENAPI_JSON_ROUTE, SovdIdentities, SovdRegistry, add_openapi_routes};
    ...
    async fn openapi_requests_are_independent_and_reflect_latest_routes() {
        ...
        let registry = SovdRegistry::new(SovdIdentities::default());
        dynamic_router
