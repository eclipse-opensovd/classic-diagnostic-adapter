...
3. Wait a few seconds for DoIP discovery, then confirm the CDA found your ECUs:
   ```shell
   curl http://localhost:8080/vehicle/v15/components | jq
   ```
4. Browse the full API interactively at **http://localhost:8080/swagger-ui**.
...
#### onboard vs. offboard tester

Use `--onboard-tester` to select the DoIP protocol variant:

| Flag | Protocol | When to use |
|------|----------|-------------|
| `--onboard-tester true` *(default)* | DoIP-DOBT | CDA runs inside the vehicle (e.g. on a gateway ECU) |
| `--onboard-tester false` | DoIP | CDA runs outside the vehicle (laptop, test bench) |

```shell
# Offboard
cargo run --release -- --databases-path ./databases --tester-address <YOUR_IP> --onboard-tester false
```
