...
///
/// The caller owns the state so that registration and mounting can be ordered
/// independently: a provider registered after the routes are mounted still
/// appears, and one registered before them is never missed. There is
/// deliberately no variant that creates the state and mounts it in one call,
/// because that hands back a state that is already reachable and empty. See
