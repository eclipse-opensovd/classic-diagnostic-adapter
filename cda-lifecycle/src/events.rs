...
impl EcuRevisions {
    ...
    /// Replaces the complete live revision set and returns the one displaced.
    #[must_use]
    pub fn replace(&self, revisions: HashMap<String, String>) -> HashMap<String, String> {
        std::mem::replace(&mut *std_ext::lock_write(&self.0), revisions)
    }
}
...
mod tests {
    ...
    /// [[ test~deferred-post-update-lease, Deferred post-update mode drops the communication lease instead of releasing it, test ]]
    #[test]
    ...
    #[test]
    fn replacing_revisions_removes_absent_ecus_and_returns_the_previous_set() {
        let revisions = EcuRevisions::default();
        revisions.record("removed", "1");
        let replacement = HashMap::from_iter([("current".to_owned(), "2".to_owned())]);

        let previous = revisions.replace(replacement.clone());

        assert_eq!(revisions.snapshot(), replacement);
        assert_eq!(previous.get("removed"), Some(&"1".to_owned()));
        assert!(!revisions.snapshot().contains_key("removed"));
    }
}
