...
async fn dispatch<E: LifecycleEvent>(
    ...
    // The HTTP protection outlives the last component and remains up when the
    ...
    match (dispatched, lease) {
        (Err(preceding), Err(LifecycleError::LeaseUnsettled { failure, .. })) => {
            Err(LifecycleError::LeaseUnsettled {
...
mod tests {
    ...
    impl Transport {
        ...
        fn fail_release(&self) {
            std_ext::lock_mutex(&self.state).fail_release = true;
        }
    ...
    impl DisableGuard for DisableLease {
        async fn release(
            ...
            if std_ext::lock_mutex(&self.state).fail_release {
                std_ext::lock_mutex(&self.state).log.push("release-failed");
                return Err(CommunicationOperationFailure::TransportFailure {
                    operation: CommunicationOperation::Resume,
                    error: "release failed".to_owned(),
                });
            }
            self.resolved = true;
    ...
    #[tokio::test]
    async fn a_dispatch_and_lease_failure_are_both_reported() {
        let transport = Transport::enabled();
        transport.fail_release();
        let harness = harness(transport, LeaseOutcome::Release, Some(TestStage::Transport));
        harness
            .register(vec![
                RecordingComponent::new("files", &harness.log).shared(),
                RecordingComponent::new("databases", &harness.log)
        ...
        let error = harness
            .run(guarded_event())
            .await
            .expect_err("the component and lease both fail");

        match error {
            LifecycleError::LeaseUnsettled {
                preceding: Some(preceding),
                ..
            } => assert!(matches!(*preceding, LifecycleError::Component { .. })),
    ...
    /// [[ test~deferred-post-update-transport, Deferred post-update mode leaves the transport down, test ]]
    #[tokio::test]
