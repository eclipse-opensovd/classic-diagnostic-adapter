...
pub(crate) struct HeldGuards {
    // Dropping the token lifts the protection.
    http: Option<Box<dyn Send>>,
...
impl HeldGuards {
    ...
    pub(crate) async fn resolve_lease(&mut self, degraded: bool) -> Result<(), LifecycleError> {
        ...
        lease
            ...
            .await
            .map(|_| ())
            .map_err(|failure| LifecycleError::LeaseUnsettled {
                preceding: None,
                failure,
            })
    }

    pub(crate) fn retain_http(&mut self) {
        if let Some(protection) = self.http.take() {
