...
pub trait PreparedApply: Send {
    ...
    /// Publishes the prepared value and returns its inverse when available.
    #[must_use]
...
/// Integrator-provided integrity validation for MDD database files.
///
/// MDD parsing, metadata access, and decompression are framework-owned common
/// operations. Implementations only supply the content-trust policy.
pub trait DatabaseValidator: Send + Sync + 'static {
    /// Verifies that `path` holds a well-formed runtime database and satisfies
    /// the application's content-trust policy, such as a signature or hash check.
    ///
    /// Called on each staged file before it is accepted, before promotion, and
    /// before an installed file is constructed into live state.
    ///
    /// # Errors
    /// Returns [`VerificationError`] when the file is malformed, unreadable, or untrusted.
...
pub trait RuntimeFileStore: Send + Sync + 'static {
    ...
    /// [`DatabaseValidator::check_integrity`] before it is accepted.
    async fn upload(
        &self,
