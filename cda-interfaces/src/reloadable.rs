...
impl<T> ReloadableOwner<T> {
    ...
    /// Replaces the data with a value derived from the current one, under one
    /// write guard so no other update can interleave.
    pub async fn apply_with(&self, derive: impl FnOnce(&T) -> T) {
        let mut current = self.0.0.write().await;
