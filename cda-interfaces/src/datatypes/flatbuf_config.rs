    ...
    fn default() -> Self {
        FlatbBufConfig {
            ...
            #[allow(
                ...
                reason = "toml parsing is using i64 as intermediate representation, so we need to \
                          set the max_apparent_size to i64::MAX to prevent out-of-range value for \
                          usize type. Truncation must be accepted, as the toml file doesn't \
                          support larger values anyway. If this turns out to be a problem, we can \
                          consider using a custom deserializer for this field or remove it from \
                          the config."
            )]
