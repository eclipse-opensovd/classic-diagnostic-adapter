...
impl HttpProtectionRegistry {
    ...
    ///
    /// The configuration is validated first, because a malformed route widens
    /// its own scope. An empty prefix matches every path, exempting or denying
    /// the whole API instead of one route.
    ///
    /// # Errors
