...
pub enum LifecycleError {
    ...
    #[error("{prefix}Communication was not restored: {failure}", prefix = preceding.as_ref().map_or_else(String::new, |error| format!("{error}; ")))]
    LeaseUnsettled {
        /// The dispatch failure that preceded finalization, when both failed.
        preceding: Option<Box<LifecycleError>>,
        /// Why the communication lease could not be settled.
        failure: crate::communication_control::CommunicationOperationFailure,
    },
