...
///
/// Whatever the reason, taking a lease deinitializes the communication
/// lifecycle, which cancels every variant detection in flight. Cancellation is
/// immediate and does not wait for a detection to reach a consistent point, so
/// an ECU whose detection was cut short keeps whatever partial variant state it
/// had reached.
///
/// Releasing the lease therefore re-initializes and detection runs again from
/// scratch. A lease holder must not assume variant state survives its lease.
