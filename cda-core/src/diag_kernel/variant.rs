...
impl<S: SecurityPlugin> VariantDetection for EcuManager<S> {
    ...
    async fn detect_variant<T: DiagServiceResponse + Sized>(
        ...
        if service_responses.is_empty() {
            ...
            // Publish unconditionally, even though the value may be
            // unchanged (e.g. an ECU offline since boot, still NotTested):
            // this is this ECU's detection attempt settling, and anything
            // awaiting its conclusion must be woken rather than left to rely
            // solely on its own bounded timeout.
            self.runtime_state
