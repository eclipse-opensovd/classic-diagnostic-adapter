...
use cda_interfaces::{
    DiagCommAction, DiagCommType, DiagServiceError, DynamicPlugin, EcuState, EcuVariant, Protocol,
...
mod tests {
    ...
    async fn detect_variant(
        ...
        let response = create_variant_response(
            "ReadVariantData",
            HashMap::from([("variant_code".to_owned(), variant_id)]),
        );
    ...
    async fn test_detect_unknown_variant() {
        ...
        let response = create_variant_response(
            "ReadVariantData",
            HashMap::from([("variant_code".to_owned(), 42)]),
        );
