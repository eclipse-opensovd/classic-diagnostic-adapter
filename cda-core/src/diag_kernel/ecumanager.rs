...
impl<S: SecurityPlugin> cda_interfaces::EcuManager for EcuManager<S> {
    ...
    async fn convert_from_uds(
        ...
        let sid = if let Some(first_byte) = payload.data.first() {
            if *first_byte != service_ids::NEGATIVE_RESPONSE {
                *first_byte
            } else if let Some(second_byte) = payload.data.get(1) {
                *second_byte
            } else {
                return Err(DiagServiceError::BadPayload(
                    "Negative response missing service id".to_owned(),
                ));
            }
        } else {
            return Err(DiagServiceError::BadPayload(
