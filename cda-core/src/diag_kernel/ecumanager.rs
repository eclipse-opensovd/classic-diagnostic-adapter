...
pub struct EcuManager<S: SecurityPlugin> {
    ...
    pub(in crate::diag_kernel) embedded_files: cda_database::FileManager,
...
impl<S: SecurityPlugin> cda_interfaces::file_manager::EmbeddedFiles for EcuManager<S> {
    async fn list(&self) -> HashMap<String, cda_interfaces::file_manager::ChunkMetaData> {
        self.embedded_files.list().await
    }
    ...
    > {
        self.embedded_files.get(id).await
    }
}
...
impl<S: SecurityPlugin> EcuManager<S> {
    ...
    pub fn new(
        ...
        embedded_files: cda_database::FileManager,
    ...
    pub(crate) async fn set_variant(&self, variant: VariantData) -> Result<(), DiagServiceError> {
        ...
        // Outside the write lock, to keep the lock scope small.
...
mod tests {
    ...
    /// A watcher subscribed before detection concludes observes the published
    /// `VariantState`. One subscribing afterwards sees it immediately via the
