...
use cda_interfaces::{
    DiagCommType, DiagServiceError, Dtc, HashMap,
...
impl<S: SecurityPlugin> Dtc for EcuManager<S> {
...
fn find_dtc_dop_in_params<'a>(
    ...
    for p in params {
        ...
        let nested_params = extract_nested_params(&dop.into())?;
        if let Some(result) = find_dtc_dop_in_params(&nested_params)? {
...
fn extract_nested_params<'a>(
    ...
    if let Some(end_of_pdu_dop) = dop.specific_data_as_end_of_pdu_field() {
        let struct_ = end_of_pdu_dop
            ...
            .ok_or_else(|| {
                DiagServiceError::InvalidDatabase("EndOfPdu does not contain a struct".to_owned())
            })?;
