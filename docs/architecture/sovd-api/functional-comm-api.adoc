...
The diagnostic description mdd file-name, in which the information is contained must be configurable. When no file is configured, functional communication is not available.
...
Since functional communication returns data from multiple ECUs, the `/data/{data-id}` endpoint must return in the top-level of `data` the name of the ECU as key, and its returned data (if any) as value.
