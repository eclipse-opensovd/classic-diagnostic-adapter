...
.. arch:: Startup Sequence
    ...
    **Component Initialization Order**
    ...
    3. **HTTP Server Phase**: Launch the web server with a dynamic router that supports
       deferred route registration. Health endpoints are registered immediately with
       status "Starting".

    4. **Health Registration Phase**: Register component-specific health providers
       ...
       health-related registration steps are skipped.
    ...
    7. **Ready Phase**: Update the main health status to "Up" indicating the CDA's HTTP API
       is operational. In deferred mode, the DoIP health provider remains in "Pending" state
       until communication initialization is triggered and completed (see
       :need:`arch~dt-deferred-initialization`).
...
.. arch:: Database Loading
    ...
    **Health Status Integration**
    ...
    A database health provider is registered with initial status "Starting". After loading completes:
...
.. arch:: DoIP Gateway Initialization
    ...
    .. note::
       ...
       occurs. The health provider for the DoIP component remains in "Pending" state until
...
.. arch:: Deferred Initialization
    ...
    **Dynamic Router Architecture**
    ...
    1. Immediate availability of health endpoints during startup
    ...
    **Pre-initialization State**
    ...
    - Health endpoints report status for available components (configuration, HTTP server)
    ...
    **Initialization Sequence**
    ...
    detection. Upon completion, SOVD routes are registered and health status transitions to "Up".
...
.. arch:: Startup Error Handling
    ...
    **Component Health Providers**
    ...
    Health monitoring is an optional build-time feature. When enabled, each major component
    registers a health provider with granular status:
    ...
    **Graceful Degradation Behaviors**
    ...
    - **Deferred initialization failure**: When deferred initialization is triggered
      ...
      UDS manager creation fails, the error is reported to the caller and the DoIP
      health provider transitions to "Failed" state. The HTTP server and non-ECU
      endpoints remain operational. Subsequent trigger attempts may retry initialization.
