...
.. arch:: Communication Timeout Context
    ...
    Timeout variants of the communication error carry a ``TimeoutContext`` struct that provides
    the information needed to diagnose the timeout without log inspection:

    * ``configured_timeout_ms`` -- the timeout duration that was configured for this phase (in
      milliseconds).
    * ``elapsed_ms`` -- the elapsed time at the point the timeout was detected (in milliseconds).

    ``TimeoutContext`` derives ``Serialize`` for automatic parameter propagation.
    ...
    .. uml::
        ...
        CommunicationError *-- TimeoutContext
        @enduml
...
.. arch:: Conversion Response Type
    ...
    * ``errors`` -- a list of ``ConversionFromPayloadError`` values, one per field that could
    ...
    .. uml::
        ...
        ConversionResponse *-- ConversionFromPayloadError -- ConversionError
        @enduml
