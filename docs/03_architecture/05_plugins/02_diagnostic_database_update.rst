...
.. arch:: Diagnostic Database Update Plugin
    ...
    **Providing a Custom Update Plugin**
    ...
    then is each opaque owner-specific payload applied, while transport is disabled.
    A custom update plugin therefore receives neither live diagnostic request authority nor
    component publication authority. See ``docs/04_adr/06_deferred_initialization.rst`` for the rationale.
    ...
    **Execution Lifecycle**
    ...
    An apply that is rejected by the runtime is not an applied update: the rejected set is returned to nextupdate,
    so that it can be corrected and reapplied without uploading it again, while the backup keeps naming the last
    known good state.
