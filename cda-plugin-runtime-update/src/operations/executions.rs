...
const SAFE_EXECUTION_FAILURE: &str = "Runtime update failed";
const SAFE_RECOVERY_FAILURE: &str = "Runtime update failed and operator recovery is required";

fn ordinary_execution_failure() -> ExecutionStatus {
    ExecutionStatus::Failed(ExecutionFailure::ordinary(SAFE_EXECUTION_FAILURE))
}

fn fatal_recovery_failure() -> ExecutionStatus {
    ExecutionStatus::Failed(ExecutionFailure::fatal(SAFE_RECOVERY_FAILURE))
}
...
/// Finishes the execution: settle communication, drop the HTTP protection, and
/// publish the terminal status last.
///
    ...
    R: RuntimeReloaderPlugin + ?Sized,
{
    ...
    let (status, finalization) = match &outcome {
        ...
        Err(RuntimeUpdateError::ReloadFailed(ReloadFailure::RecoveryRequired {
            ...
            (fatal_recovery_failure(), Finalization::Quarantine)
        }
        Err(error) => {
            tracing::error!(
                execution_id = %task.execution_id,
...
mod tests {
    ...
    use cda_interfaces::{
        ...
        runtime_update_api::{
            ExecutionMode, ExecutionStatus, RecoveryPhase, ReloadError, ReloadFailure,
            RuntimeReloaderPlugin, RuntimeUpdateError, UpdateExecution,
        },
    ...
    async fn failed_compensation_retains_protection_and_requires_recovery() {
        ...
        let status = poll_until_terminal(&f.executions, &execution_id).await;
        assert_eq!(status, super::fatal_recovery_failure());
        assert!(
            f.http_restriction_manager.is_active(),
