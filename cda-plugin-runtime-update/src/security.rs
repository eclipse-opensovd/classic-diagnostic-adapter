...
use cda_interfaces::{
    runtime_update_api::{
        ...
        UpdateFileType, VerificationError,
    },
...
impl<L: LockStateProvider> DefaultUpdateSecurityHandler<L> {
    /// Creates a security handler. Communication activity is admitted atomically by
    /// the communication coordinator before this handler runs.
    pub fn new(_lock_provider: Arc<L>) -> Self {
...
mod tests {
    ...
    fn make_handler(
        ...
        let handler = DefaultUpdateSecurityHandler::new(Arc::new(MockLockProvider {
            owner: owner.map(ToOwned::to_owned),
            has_ecu_conflicts,
            has_fg_conflicts,
        }));
