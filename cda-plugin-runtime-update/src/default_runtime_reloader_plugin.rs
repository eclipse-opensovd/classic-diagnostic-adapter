...
use cda_interfaces::{
    runtime_update_api::{
        ApplicationUpdatePreparation, RecoveryError, RejectedSetDisposition, ReloadError,
        ReloadFailure, RuntimeReloaderPlugin,
    },
    storage_api::Storage,
};
    ...
    S: Storage + Send + Sync + 'static,
{
    async fn reload_databases(
        ...
        let restored = match on_reject {
            RejectedSetDisposition::Swap => {
                crate::operations::rollback::restore_backup(&*self.storage).await
            }
            RejectedSetDisposition::Restage => {
                crate::operations::rollback::restore_backup_and_restage_rejected(&*self.storage)
                    .await
            }
        };
        if let Err(recovery) = restored {
            return Err(ReloadFailure::RecoveryFailed {
                original,
        ...
        match self.prepare_once(&config).await {
            ...
            Err(recovery) => Err(ReloadFailure::RecoveryFailed {
                original,
                recovery: RecoveryError::RestoredPreparation(recovery),
            }),
        }
    }
}
...
mod tests {
    ...
    /// A rejected rollback must leave the pre-action state intact, and a staged
    /// `NextUpdate` is part of that state: the requirement that a rollback
    /// "also clears pending nextupdate" describes a rollback that succeeded.
    #[tokio::test]
    async fn rejected_rollback_leaves_pending_next_update_untouched() {
        ...
        .await;
        let (plugin, _, _staged) = plugin(Arc::clone(&storage), true, false);

        let result = crate::operations::rollback::execute_rollback(&*storage, &plugin).await;
        assert!(matches!(
            result,
            Err(RuntimeUpdateError::ReloadFailed(
                ReloadFailure::RejectedAndRestored { .. }
            ))
        ));
        assert_eq!(
            bytes(&storage, CollectionName::DiagnosticDatabaseNextUpdate).await,
            PENDING,
            "a rejected rollback must not consume the staged update"
        );
    }
    ...
    async fn explicit_rollback_reports_a_and_b_failure_and_keeps_b_current() {
        ...
        let result = crate::operations::rollback::execute_rollback(&*storage, &plugin).await;
        let Err(RuntimeUpdateError::ReloadFailed(ReloadFailure::RecoveryFailed {
            original,
            recovery,
            ..
        })) = result
    ...
    /// A rejected apply is not an applied update: the rejected set goes back to
    /// staging for correction, the previously live set comes back, and the backup
    /// keeps naming the last known good state rather than the rejected one.
    #[tokio::test]
    async fn rejected_apply_restages_candidate_and_keeps_good_set_as_backup() {
        let directory = tempfile::tempdir().unwrap();
        let storage = Arc::new(LocalStorage::new(directory.path()).unwrap());
        seed(&storage, A_VALID, A_VALID).await;
        crate::test_utils::init_collection(
            &storage,
            &CollectionName::DiagnosticDatabaseNextUpdate,
            &[("ecu.mdd", B_INVALID)],
        ...
        .await;
        let (plugin, _, staged) = plugin(Arc::clone(&storage), false, true);

        let result = crate::operations::apply::execute_apply(&*storage, &plugin).await;

        assert!(matches!(
            result,
            Err(RuntimeUpdateError::ReloadFailed(
                ReloadFailure::RejectedAndRestored { original }
            )) if original.to_string().contains("B-invalid")
        ));
        assert_eq!(*staged.lock().unwrap(), vec![A_VALID.to_vec()]);
        assert_eq!(
            bytes(&storage, CollectionName::DiagnosticDatabase).await,
        ...
        assert_eq!(
            bytes(&storage, CollectionName::DiagnosticDatabaseBackup).await,
            A_VALID,
            "the backup must keep the last known good set, not the rejected one"
        );
        assert_eq!(
            bytes(&storage, CollectionName::DiagnosticDatabaseNextUpdate).await,
            B_INVALID,
            "the rejected set must be correctable in staging"
        );
    }
}
