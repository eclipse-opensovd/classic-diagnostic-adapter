...
def add_admin_data(dlc: DiagLayerContainer):
    dlc.admin_data = AdminData(
        doc_revisions=[DocRevision(revision_label="00.01.00", date=f"{datetime.datetime.now().isoformat()}")]
    )
...
def add_functional_classes(dlr: DiagLayerRaw):
    funct_class_names = ["Session", "EcuReset", "CommCtrl", "Ident", "StandardDataTransfer"]
    dlr.functional_classes = NamedItemList([
        FunctionalClass(
            odx_id=OdxLinkId(local_id=f"{dlr.odx_id.local_id}.FNC.{name}", doc_fragments=dlr.odx_id.doc_fragments),
            short_name=name,
...
def add_additional_audiences(dlc: DiagLayerContainer):
    add_audiences_names = ["Anyone", "After_Sales", "Manufacturing", "After_Market", "Development", "Supplier", "Custom"]
    dlc.additional_audiences = NamedItemList([
        AdditionalAudience(
            odx_id=OdxLinkId(local_id=f"{dlc.odx_id.local_id}..AA", doc_fragments=dlc.odx_id.doc_fragments),
            short_name=name,
