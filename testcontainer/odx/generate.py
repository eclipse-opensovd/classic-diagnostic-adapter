...
from odxtools.odxlink import OdxLinkId, OdxLinkRef, DocType, OdxDocFragment
from odxtools.parentref import ParentRef

from comparams import generate_comparam_refs
from metadata import add_functional_classes, add_admin_data, add_company_datas, add_additional_audiences
from reset import add_reset_services
from security_access import add_security_access_services
from shared import add_common_datatypes, \
    add_state_charts, add_common_diag_comms


def add_variant(dlc: DiagLayerContainer,
                name: str,
                identification_pattern: int
                ):
    ...
    variant = EcuVariantRaw(
        ...
        ecu_variant_patterns=[
            EcuVariantPattern(matching_parameters=[
                MatchingParameter(
        ...
        parent_refs=[
            ParentRef(layer_ref=OdxLinkRef.from_id(dlc.base_variants[0].odx_id))
        ],
...
def add_base_variant(dlc: DiagLayerContainer,
                     logical_address: int,
                     gateway_address: int,
    ...
    base_variant = BaseVariantRaw(
        ...
        parent_refs=[
            ParentRef(layer_ref=OdxLinkRef.from_id(
                OdxLinkId(
                    local_id="PROTO.UDS_Ethernet_DoIP",
                    doc_fragments=(OdxDocFragment(
...
def generate_for_ecu(ecu_name: str,
                     logical_address: int,
                     gateway_address: int,
                     functional_address: int):
    ...
    add_variant(
        dlc=dlc,
        name=f"{ecu_name}_Boot_Variant",
        identification_pattern=0xFF0000
    )

    add_variant(
        dlc=dlc,
        name=f"{ecu_name}_App_0101",
        identification_pattern=0x000101
    )
