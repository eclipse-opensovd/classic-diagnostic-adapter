...
impl Message<SetDefaultStates> for EcuCoordinator {
    ...
    async fn handle(
        ...
        let mut ss = std_ext::lock_write(&self.state.service_states);
        for (sid, default_value) in msg.defaults {
            ss.entry(sid).or_insert(default_value);
        }
    }
}
...
/// ECU disconnected event -> sets connectivity to Offline.
///
/// Only transitions from `Online` to `Offline`. If already `Offline`, this is a no-op.
/// Variant state is **intentionally untouched**, as the ECU's detected variant remains
/// valid for the next reconnection attempt.
...
impl Message<EcuDisconnected> for EcuCoordinator {
    ...
    async fn handle(
        ...
        if es.connectivity == Connectivity::Online {
            tracing::info!(
                ecu = %self.ecu_name,
                dlt_context = dlt_ctx!("UDS"),
                "ECU disconnected. Setting connectivity to Offline"
            );
            es.connectivity = Connectivity::Offline;
        } else {
            tracing::debug!(
                ecu = %self.ecu_name,
...
mod tests {
    ...
    async fn ecu_disconnected_preserves_variant() {
        ...
        {
            let mut es = handle.state.ecu_state.write().unwrap();
            es.connectivity = Connectivity::Online;
    ...
    #[tokio::test]
    async fn clear_variant_for_redetect_sets_not_detected() {
        let handle = spawn_test_coordinator("TestECU");
        {
            let mut es = handle.state.ecu_state.write().unwrap();
            es.connectivity = Connectivity::Online;
        ...
        let status = handle.ecu_status();
        assert_eq!(status.variant_state, VariantState::NotDetected);
