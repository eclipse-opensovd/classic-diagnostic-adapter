...
//! permits a lifecycle request. It only reads [`CommunicationStateData`]; each
//! caller applies the returned [`LifecycleDecision`] with its own state updates
//! and error handling.
//!
//! This module decides state admission only. Publishing an admitted operation's
//! outcome and checking caller authority remain the responsibility of callers.
...
pub(crate) enum LifecycleDecision {
    ...
    /// Communication is already enabled. No activation or detection stage runs;
    /// use [`Detect`](CommunicationOperation::Detect) to detect on a live
    /// transport.
    AlreadyEnabled,
...
///
/// This function reads state only. The caller applies the returned decision
/// while holding the same lock.
///
/// The exhaustive match below requires an explicit decision for each state and
/// request combination.
pub(crate) fn decide(
    ...
    match (&state.state, request) {
        // These requests are already satisfied; they do not trigger detection.
        (
        ...
        // Detection requires an already-live transport and never starts one.
        (
...
///
/// Guard acquisition is separate from lifecycle transitions. Detection does not
/// affect guard admission because communication remains enabled during a sweep.
...
mod tests {
    ...
    /// Held guards refuse disable from every state.
    #[test]
    ...
    /// Disable conflicts with an in-flight activation.
    #[test]
    ...
    /// Detection requires a live transport, so it cannot join activation.
    #[test]
