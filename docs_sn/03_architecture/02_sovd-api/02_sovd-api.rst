...
Paths are required to be in the following structure: ``/bulk-data/{category}/{id}``. For extensions, the name ``bulk-data`` may only be used in the end of a path element.
...
.. list-table:: Bulk Data endpoints
   ...
   * - GET
     ...
     - Downloads the data for the entry. The mime type is determined by the server, and content of the data.
   * - POST
     - ``/bulk-data/{category}``
...
This doesn't include the ``mdd`` files used for functional communication (see ../../architecture/03_sovd-api/03_extensions/02_functional-comm-api.adoc).

ECU resource collection
^^^^^^^^^^^^^^^^^^^^^^^

The resource collection for ECUs is defined in an OpenAPI Specification: ../../architecture/03_sovd-api/openapi/ecu_resource_collection.yaml (ECU Resource Collection Specification)
...
ECUs state chart for the SID 10\ :sub:`16` services. It is case-insensitive.
...
Works similar to Session defined in the previous chapter. The names
of the security access levels are determined through the state charts for the SID 27\ :sub:`16` services.
...
.. todo:: Faults (SID 14\ :sub:`16` & 19\ :sub:`16`)
