...
use crate::{
    ...
    connections::{
        EcuError::EcuConnectionError,
        receiver::{handle_response, handle_send_pending},
    },
...
mod receiver {
    ...
    pub(super) async fn handle_response(
        ...
        match result {
            ...
            Err(err) => handle_connection_error(gateway_name, gateway_ip, reset_tx, err, ecu_names, connectivity_handler).await,
        }
    }
    ...
    mod tests {
        ...
        async fn decode_error_does_not_trigger_reset() {
            ...
            let connectivity_handler: Arc<dyn EcuConnectivityHandler> = Arc::new(MockConnectivityHandler);
        ...
        struct MockConnectivityHandler;

        #[async_trait::async_trait]
        impl EcuConnectivityHandler for MockConnectivityHandler {
