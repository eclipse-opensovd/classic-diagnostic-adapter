...
use doip_definitions::payload::{
    ActivationType, AliveCheckResponse, DoipPayload, RoutingActivationRequest,
};
use thiserror::Error;
use tokio::{
...
/// Handlers for incoming `DoIP` gateway messages
mod receiver {
    use super::*;
    ...
    pub(super) async fn handle_response(
        ...
        match result {
            ...
            Err(err) => handle_connection_error(gateway_name, gateway_ip, reset_tx, err).await,
        }
    }
    ...
    /// Acquires the read-half of the gateway connection and waits for the next
    /// incoming message.  Returns `None` when the read lock could not be
    /// acquired (connection being reset) or when `try_read` times out.
    pub(super) async fn read_response<T>(
        gateway_conn: &Arc<EcuConnectionTarget<T>>,
        ...
        T: AsyncRead + AsyncWrite + Unpin + Send + 'static,
    {
        ...
        // We can wait without an explicit timeout here because the select! in the
        // outer loop will cancel this future as soon as send_pending_rx fires.
    ...
    mod tests {
        ...
        #[tokio::test]
        async fn decode_error_does_not_trigger_reset() {
            let (reset_tx, mut reset_rx) = mpsc::channel(4);
            handle_connection_error(
                "gw",
                "1.2.3.4",
                &reset_tx,
                ConnectionError::Decoding("bad frame".to_owned()),
            )
            .await;
            assert!(
                reset_rx.try_recv().is_err(),
                "decode error must not trigger a connection reset"
            );
        }
    }
}
    ...
    T: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    cda_interfaces::spawn_named!(&format!("gateway-receiver-{gateway_ip}"), async move {
        'receive: loop {
            ...
            // Tokio randomises which branch takes precedence when both are ready,
            // which prevents send/receive starvation.
            tokio::select! {
                send_pending_result = send_pending_rx.changed() => {
                    if receiver::handle_send_pending(
                        &gateway_name,
                        &gateway_ip,
                        &mut send_pending_rx,
                        send_pending_result,
                ...
                response = receiver::read_response(&gateway_conn) => {
