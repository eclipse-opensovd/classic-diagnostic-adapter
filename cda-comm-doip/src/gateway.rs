...
#[derive(Debug, Clone)]
pub(crate) enum DiagnosticResponse {
...
pub(crate) struct DoipEcu {
    pub(crate) sender: mpsc::Sender<DoipPayload>,
    pub(crate) receiver: broadcast::Receiver<Result<DiagnosticResponse, EcuError>>,
}

pub(crate) struct DoipConnection {
    pub(crate) ecus: HashMap<u16, Arc<Mutex<DoipEcu>>>,
    pub(crate) ip: String,
}
...
impl<T: EcuAddresses + DoipComParams> DoipDiagGateway<T> {
        ...
        F: Future<Output = ()> + Send + 'static,
    {
        ...
        let gateways = crate::vir_vam::get_vehicle_identification::<T, F>(
        ...
        let state = if gateways.is_empty() {
            ...
            for gateway in gateways {
                if let Ok(logical_address) = crate::connections::handle_gateway_connection::<T>(
                    gateway,
                    &transport_config,
                    &GatewayState {
        ...
        let vam_listener_handle = crate::vir_vam::listen_for_vams(
            transport_config,
            mask,
            state.clone(),
            variant_detection,
