...
use cda_interfaces::{
    DiagServiceError, DoipComParams, EcuAddresses, EcuGatewaySockets, FunctionalTransport, HashMap,
...
impl<T: EcuAddresses + DoipComParams> NetworkTopology for DeferredGateway<T> {
...
impl<T: EcuAddresses + DoipComParams> PhysicalTransport for DeferredGateway<T> {
    async fn shutdown(&mut self) {
        cda_interfaces::Shutdown::shutdown(self).await;
    }
...
impl<T: EcuAddresses + DoipComParams> FunctionalTransport for DeferredGateway<T> {
...
impl<T: EcuAddresses + DoipComParams> TransportProbe for DeferredGateway<T> {
    async fn route_status(&self, ecu_name: &str) -> RouteStatus {
        let guard = self.slot.read().await;
        match guard.as_ref() {
            Some(gateway) => gateway.route_status(ecu_name).await,
            None => RouteStatus::Unavailable,
        }
    }

    async fn probe(&self, ecu_name: &str) -> bool {
        let guard = self.slot.read().await;
        match guard.as_ref() {
            Some(gateway) => gateway.probe(ecu_name).await,
            None => false,
        }
    }
}
