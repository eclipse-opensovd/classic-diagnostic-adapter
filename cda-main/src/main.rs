...
impl AppArgs {
    ...
    fn update_config(self, config: &mut opensovd_cda_lib::config::configfile::Configuration) {
        ...
        #[cfg(feature = "health")]
        if let Some(heal_listen_address) = self.health_listen_address {
            config.health.address = heal_listen_address;
        }
        #[cfg(feature = "health")]
        if let Some(health_listen_port) = self.health_listen_port {
            config.health.port = health_listen_port;
        }
    }
}
