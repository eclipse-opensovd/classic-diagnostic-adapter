...
use cda_interfaces::{
    DiagServiceError, DoipGatewaySetupError, EcuAddresses, EcuConnectivityHandler,
    EcuManager as EcuManagerTrait, EcuManagerType, FunctionalDescriptionConfig, HashMap,
    HashMapEntry, HashMapExtensions, HashSet, Protocol, UdsQuery, UdsVariant,
...
impl AppArgs {
    #[tracing::instrument(skip(self, config), fields(dlt_context = dlt_ctx!("MAIN")))]
...
impl From<DoipGatewaySetupError> for AppError {
    fn from(value: DoipGatewaySetupError) -> Self {
        match value {
            ...
            DoipGatewaySetupError::UnknownECU { .. } => Self::ConfigurationError(value.to_string()),
...
>(
    ...
    let state_coordinator = create_state_coordinator(&databases).await;
    let doip_socket = Arc::new(Mutex::new(
    ...
    let diagnostic_gateway = match create_diagnostic_gateway(
        ...
        Arc::new(state_coordinator.clone()) as Arc<dyn EcuConnectivityHandler>,
        clonable_shutdown_signal.clone(),
        Arc::clone(&doip_socket),
        health,
    )
    .await
    {
...
pub async fn load_databases<S: SecurityPlugin>(
    ...
    let strict_parameter_validation = config.strict.parameter_validation();
...
async fn create_state_coordinator<S: SecurityPlugin>(
    ...
    for (ecu_name, db_lock) in databases {
        let db = db_lock.read().await;
        runtime_states.insert(ecu_name.clone(), db.runtime_state());
    }

    EcuStateCoordinator::new(runtime_states)
}
...
pub fn create_uds_manager<S: SecurityPlugin>(
    ...
    let update_in_progress = Arc::new(std::sync::atomic::AtomicBool::new(false));
    UdsManager::new(
        gateway,
        databases,
...
#[tracing::instrument(
    skip(databases, variant_detection, connectivity_handler, shutdown_signal, doip_socket, health),
    fields(
        database_count = databases.len(),
        dlt_context = dlt_ctx!("MAIN"),
    )
...
pub async fn run_with_config(config: Configuration) -> Result<(), AppError> {
    ...
    #[cfg(feature = "health")]
    let health_state = if config.health.enabled {
        Some(cda_health::add_health_routes(&dynamic_router, cda_version().to_owned()).await)
    } else {
        None
    };

    #[cfg(not(feature = "health"))]
    let health_state: Option<cda_health::HealthState> = None;

    let vehicle_data = load_vehicle_data::<_, DefaultSecurityPluginData>(
        &config,
        clonable_shutdown_signal.clone(),
        health_state.as_ref(),
    )
    ...
    let _ = cda_sovd::add_vehicle_routes::<_, _, DefaultSecurityPlugin>(
        &dynamic_router,
    ...
    tracing::info!("CDA initialized and ready to serve requests");
