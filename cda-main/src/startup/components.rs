...
use cda_interfaces::{
    ...
    runtime_update_api::{
        PreparedApply, ReloadError, RuntimeFileTransaction, RuntimeFilesUpdatePlugin,
...
struct PreviousVehicleData<SP: SecurityPlugin> {
    lock_topology: Option<Box<dyn PreparedApply>>,
    ecu_data: Option<VehicleEcuData<EcuManager<SP>>>,
    can_topology: Option<crate::vehicle::CanTopologyPayload>,
    sovd_registry: SovdRegistrySnapshot,
    revisions: cda_interfaces::HashMap<String, String>,
}

fn discard_rejected<T>(_rejected: T) {}
...
impl<SP: SecurityPlugin> CommitEcuData<SP> {
    ...
    async fn install(&self, reload: &EcuDataReload) -> Result<(), LifecycleError> {
        ...
        let previous = PreviousVehicleData {
            lock_topology: lock_reservation.apply_reversible(),
            ecu_data: self.uds_reload.swap(ecu_data).await,
            can_topology: self.can_reload.swap(can_topology).await,
            sovd_registry: self.sovd_registry.swap_for_update(identities).await,
            revisions: self.revisions.replace(revisions),
        };
        reload.outgoing.retain(previous);
        Ok(())
    }
}
...
impl<SP: SecurityPlugin> ConstructedComponent<CdaEvent> for CommitEcuData<SP> {
    ...
    /// A later component failed, so every live view goes back to what it
    /// displaced.
    async fn revert(&self, event: &CdaEvent) -> Result<(), LifecycleError> {
        let CdaEvent::ReloadEcuData(reload) = event else {
            return Ok(());
        };
        let Some(previous) = reload.outgoing.take::<PreviousVehicleData<SP>>() else {
            return Ok(());
        };
        let rejected_revisions = self.revisions.replace(previous.revisions);
        drop(rejected_revisions);
        let rejected_sovd_registry = self.sovd_registry.restore(previous.sovd_registry).await;
        discard_rejected(rejected_sovd_registry);
        let rejected_can_topology = self.can_reload.swap(previous.can_topology).await;
        discard_rejected(rejected_can_topology);
        if let Some(ecu_data) = previous.ecu_data {
            self.uds_reload.apply(ecu_data).await;
        }
        if let Some(lock_topology) = previous.lock_topology {
            lock_topology.apply();
        }
        Ok(())
    }
...
impl<SP: SecurityPlugin> LoadEcuData<SP> {
    ...
    async fn prepare(&self, reload: &EcuDataReload) -> Result<(), LifecycleError> {
        ...
        let previous_event_revisions = reload.revisions.replace(prepared.revisions.clone());
        drop(previous_event_revisions);
    ...
    async fn load(&self) -> Result<(), LifecycleError> {
        ...
        let empty_revisions = self
            .revisions
            .replace(self.loader.revisions(&self.config).await);
        drop(empty_revisions);

        let ecu_names = ecu_data.physical_ecu_names();
        self.targets.uds.apply(ecu_data).await;
        self.targets.sovd.apply(identities).await;
        let empty_can_topology = self.targets.can.swap(can_topology).await;
        discard_rejected(empty_can_topology);
