...
impl CanReload {
    ...
    pub(crate) async fn swap(
        &self,
        topology: Option<CanTopologyPayload>,
    ) -> Option<CanTopologyPayload> {
        match (self.0.as_ref(), topology) {
            (Some(owner), Some(topology)) => owner.swap(topology).await,
            (None, None) => None,
            _ => {
                tracing::error!("Prepared CAN topology does not match the configured CAN owner");
                None
            }
        }
    }
    ...
    pub(crate) async fn swap(
        &self,
        _topology: Option<CanTopologyPayload>,
    ) -> Option<CanTopologyPayload> {
        None
    }
}
