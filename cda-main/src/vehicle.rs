...
use cda_interfaces::{
    ...
    health::HealthProvider, runtime_update_api::DatabaseValidator,
};
...
mod tests {
    ...
    fn config_for(database_dir: &std::path::Path, storage_dir: &std::path::Path) -> Configuration {
        ...
        config.database.dir = database_dir.to_string_lossy().into_owned();
    ...
    /// collection is authoritative, so resolution never falls back to the
    /// configured database dir.
    #[tokio::test]
    async fn existing_empty_storage_does_not_fall_back_to_the_database_dir() {
        ...
        for _ in 0..2 {
            let resolved =
                mdd::resolve_mdd_paths(&storage, &database_dir.path().to_string_lossy()).await;
            assert!(
                resolved.is_empty(),
    ...
    async fn construct_gateway_for_can_presence(
        ...
        config.can = Some(CanConfig {
            ecu_mappings: vec![cda_comm_can::config::CanEcuMapping {
                ...
                request_id: 0x7E0,
                response_id: 0x7E8,
                addressing_mode: cda_comm_can::config::CanAddressingMode::Standard,
            }],
            ..CanConfig::default()
        });
