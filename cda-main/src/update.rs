...
use cda_interfaces::{
    ...
    runtime_update_api::{
        AcceptedUpdate, DatabaseValidator, ExclusiveRuntimePlugin, ExecutionFailure, ExecutionMode,
        RecoveryError, ReloadError, RuntimeFilesUpdatePlugin, RuntimeUpdateError, UpdateDispatcher,
    },
    storage_api::Storage,
};
use cda_lifecycle::{
...
fn execution_failure(error: LifecycleError) -> ExecutionFailure {
    match error {
        ...
        LifecycleError::LeaseUnsettled { preceding, failure } => {
            ExecutionFailure::CommunicationFinalizationFailed {
                preceding: preceding.map(|error| Box::new(execution_failure(*error))),
                failure,
            }
        }
    }
...
where
    S: Storage + 'static,
{
    // The application supplies the integrity policy; the plugin stays agnostic.
    let database_validator = resources.database_validator;
...
mod tests {
    use cda_interfaces::communication_control::{
        CommunicationOperation, CommunicationOperationFailure,
    };

    use super::*;

    #[test]
    fn lease_finalization_failure_preserves_the_preceding_dispatch_failure() {
        let preceding = LifecycleError::Component {
            component: "database-files",
            phase: "reload".to_owned(),
            source: std::io::Error::other("reload failed").into(),
        };
        let failure = CommunicationOperationFailure::TransportFailure {
            operation: CommunicationOperation::Resume,
            error: "resume failed".to_owned(),
        };

        let mapped = execution_failure(LifecycleError::LeaseUnsettled {
        ...
        match mapped {
            ExecutionFailure::CommunicationFinalizationFailed {
                ...
                ..
            } => assert!(matches!(*preceding, ExecutionFailure::RuntimeUnchanged(_))),
