...
//!
//! Provides the runtime-update plugin with MDD validation, metadata, revision,
//! and decompression operations. Startup parsing remains in `cda-main`.
...
mod tests {
    ...
    // The inspector adds no signature policy of its own, so `check_integrity`
    // has to be exactly as strict as `validate`.
    #[test]
    fn check_integrity_rejects_a_file_that_is_not_an_mdd() {
        ...
        assert!(MddFileInspector.check_integrity(&path).is_err());
    }
